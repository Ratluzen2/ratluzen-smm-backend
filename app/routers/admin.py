...
def pending_services(db: Session = Depends(get_db)):
    ...
    return _rows(lst)  # JSONArray كما يتوقع التطبيق
...
def orders_approve(order_id: int = Body(..., embed=True), db: Session = Depends(get_db)):
    ...
    db.add(Notice(
        title="تم تنفيذ طلبك",
        body=f"تم الإرسال للمزوّد | رقم: {order.provider_order_id}",
...
def orders_reject(order_id: int = Body(..., embed=True), db: Session = Depends(get_db)):
    ...
    if u:
        u.balance = round((u.balance or 0) + (order.price or 0), 2)
        db.add(u)
    db.add(order)
    db.add(Notice(title="تم رفض الطلب", body="تم ردّ الرصيد إلى محفظتك.", for_owner=False, uid=order.uid))
    db.commit()
    return {"ok": True}
...
def accept_card(
    ...
    u.balance = round((u.balance or 0) + float(amount_usd), 2)
...
def deliver_itunes(
    oid: int = Body(..., embed=True),
    gift_code: str = Body(..., embed=True),
    db: Session = Depends(get_db)
):
...
def deliver_phone(
    ...
    o.code = code
    db.add(o)
    db.add(Notice(title="كود الشحن", body=f"الكود: {code}", for_owner=False, uid=o.uid))
    db.commit()
    return {"ok": True}

@r.post("/phone/reject", dependencies=[Depends(guard)])
def reject_phone(oid: int = Body(..., embed=True), db: Session = Depends(get_db)):
    o = db.get(PhoneTopup, oid)
    if not o or o.status != "pending":
        raise HTTPException(404, "not found or not pending")
    o.status = "rejected"
    db.add(o)
    db.add(Notice(title="رفض الشحن", body="تم رفض طلب تعبئة الرصيد.", for_owner=False, uid=o.uid))
    db.commit()
    return {"ok": True}

# ---------- PUBG ----------
@r.post("/pubg/deliver", dependencies=[Depends(guard)])
def deliver_pubg(oid: int = Body(..., embed=True), db: Session = Depends(get_db)):
    o = db.get(PubgOrder, oid)
    if not o or o.status != "pending":
        raise HTTPException(404, "not found or not pending")
    o.status = "delivered"
    db.add(o)
    db.add(Notice(title="تم التسليم - PUBG", body="تم تنفيذ طلب ببجي.", for_owner=False, uid=o.uid))
    db.commit()
    return {"ok": True}

@r.post("/pubg/reject", dependencies=[Depends(guard)])
def reject_pubg(oid: int = Body(..., embed=True), db: Session = Depends(get_db)):
    o = db.get(PubgOrder, oid)
    if not o or o.status != "pending":
        raise HTTPException(404, "not found or not pending")
    o.status = "rejected"
    db.add(o)
    db.add(Notice(title="رفض ببجي", body="تم رفض الطلب.", for_owner=False, uid=o.uid))
    db.commit()
    return {"ok": True}

# ---------- LUDO ----------
@r.post("/ludo/deliver", dependencies=[Depends(guard)])
def deliver_ludo(oid: int = Body(..., embed=True), db: Session = Depends(get_db)):
    o = db.get(LudoOrder, oid)
    if not o or o.status != "pending":
        raise HTTPException(404, "not found or not pending")
    o.status = "delivered"
    db.add(o)
    db.add(Notice(title="تم التسليم - Ludo", body="تم تنفيذ طلب لودو.", for_owner=False, uid=o.uid))
    db.commit()
    return {"ok": True}

@r.post("/ludo/reject", dependencies=[Depends(guard)])
def reject_ludo(oid: int = Body(..., embed=True), db: Session = Depends(get_db)):
    o = db.get(LudoOrder, oid)
    if not o or o.status != "pending":
        raise HTTPException(404, "not found or not pending")
    o.status = "rejected"
    db.add(o)
    db.add(Notice(title="رفض لودو", body="تم رفض الطلب.", for_owner=False, uid=o.uid))
    db.commit()
    return {"ok": True}

# ---------- المستخدمون: إحصاءات وشحن/خصم ----------
...
def users_balances(db: Session = Depends(get_db)):
    ...
    return {"list": [{"uid": u.uid, "balance": float(u.balance or 0), "is_banned": bool(getattr(u, "is_banned", False))} for u in users]}

@r.post("/users/topup", dependencies=[Depends(guard)])
def users_topup(req: AmountReq, db: Session = Depends(get_db)):
    u = db.query(User).filter(User.uid == req.uid).first()
    if not u:
        u = User(uid=req.uid, balance=0.0)
    u.balance = round((u.balance or 0) + req.amount, 2)
    db.add(u); db.commit()
    return {"ok": True, "balance": u.balance}

@r.post("/users/deduct", dependencies=[Depends(guard)])
def users_deduct(req: AmountReq, db: Session = Depends(get_db)):
    u = db.query(User).filter(User.uid == req.uid).first()
    if not u:
