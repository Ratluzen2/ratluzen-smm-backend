...
# =========================
# Code Pools with Categories (iTunes + Phone Cards)
# =========================
...
# -------- Admin: iTunes codes
class _CodesIn(BaseModel):
    codes: List[str]
    category: Optional[str] = None

def _norm_code(c: str) -> str:
    s = (c or "").strip()
    # strip RTL/LTR and spaces
    s = s.replace("\\u200f","").replace("\\u200e","").replace("\\u202a","").replace("\\u202b","").replace("\\u202c","").replace("\\u202d","").replace("\\u202e","")
    return re.sub(r"\\s+","",s)

@app.post("/api/admin/codes/itunes/add")
def api_admin_codes_itunes_add(body: _CodesIn, x_admin_password: Optional[str] = Header(None, alias="x-admin-password"), password: Optional[str] = None):
    _require_admin(_pick_admin_password(x_admin_password, password, body.dict()) or "")
    codes = [ _norm_code(x) for x in (body.codes or []) if str(x or "").strip() ]
    if not codes:
...
def api_admin_codes_itunes_list(x_admin_password: Optional[str] = Header(None, alias="x-admin-password"), password: Optional[str] = None, status: str = "unused", limit: int = 200, offset: int = 0):
    _require_admin(x_admin_password or password or "")
    status = (status or "unused").lower().strip()
    where = ["TRUE"]
    if status in ("unused","free","available"): where.append("used=FALSE")
    elif status in ("used","taken"): where.append("used=TRUE")
    conn = get_conn()
    try:
        with conn, conn.cursor() as cur:
            _ensure_itunes_codes_table(cur)
...
# -------- Admin: Phone cards codes
@app.post("/api/admin/codes/cards/{telco}/add")
def api_admin_codes_cards_add(telco: str, body: _CodesIn, x_admin_password: Optional[str] = Header(None, alias="x-admin-password"), password: Optional[str] = None):
    _require_admin(_pick_admin_password(x_admin_password, password, body.dict()) or "")
...
def api_admin_codes_cards_list(telco: str, x_admin_password: Optional[str] = Header(None, alias="x-admin-password"), password: Optional[str] = None, status: str = "unused", limit: int = 200, offset: int = 0):
    _require_admin(x_admin_password or password or "")
    telco = (telco or "").strip().lower()
    if telco not in ("atheir","asiacell","korek"):
        raise HTTPException(422, "invalid telco")
...
# =========================
# Scoped Auto-Exec (itunes, cards) with categories
# =========================
...
def _parse_category_from_title(title: str) -> Optional[str]:
    # look for numbers 5/10/15/... optionally followed or preceded by $
    t = (title or "").lower()
    m = re.search(r"(5|10|15|20|25|30|40|50|100)\s*\\$|\\$\\s*(5|10|15|20|25|30|40|50|100)", t)
    if m:
        return m.group(1) or m.group(2)
    # fallback: digits alone
...
# ---- iTunes selection and process
def _itunes_pick_pending_one_locked(cur):
    cur.execute("""
    ...
    return {"order_id": int(r[0]), "user_id": int(r[1]), "title": title, "payload": r[3], "category": category}

def _itunes_pick_code_locked(cur, category: str):
    cur.execute("""
        SELECT id, code FROM public.itunes_codes
...
def _itunes_auto_process_one(conn):
    with conn, conn.cursor() as cur:
        _ensure_itunes_codes_table(cur)
        rec = _itunes_pick_pending_one_locked(cur)
        ...
        is_jsonb = _payload_is_jsonb(conn)
        if is_jsonb and isinstance(payload, dict):
...
# ---- Cards selection and process
def _cards_pick_pending_one_locked(cur):
    cur.execute("""
        SELECT o.id, o.user_id, o.title, COALESCE(o.payload, '{}'::jsonb)
        FROM public.orders o
        WHERE COALESCE(o.status,'Pending')='Pending'
          AND o.type='topup_card'
        ORDER BY o.id ASC
        FOR UPDATE SKIP LOCKED
        LIMIT 1
    """)
    ...
    category = _parse_category_from_title(title) or "generic"
    return {"order_id": int(r[0]), "user_id": int(r[1]), "title": title, "payload": r[3], "telco": tel, "category": category}

def _cards_pick_code_locked(cur, telco: str, category: str):
    cur.execute("""
        SELECT id, code FROM public.card_codes
...
def _cards_auto_process_one(conn):
    with conn, conn.cursor() as cur:
        _ensure_card_codes_table(cur)
        rec = _cards_pick_pending_one_locked(cur)
        if not rec: return None
        if not rec["telco"]:
...
# ---- Scoped status & toggle
@app.get("/api/admin/auto_exec/status")
def admin_auto_exec_status_scoped(x_admin_password: Optional[str] = Header(None, alias="x-admin-password"), password: Optional[str] = None, scope: Optional[str] = None):
    _require_admin(_pick_admin_password(x_admin_password, password) or "")
    ...
    try:
        with conn, conn.cursor() as cur:
            ...
            if scope == "itunes":
                ...
                free_codes = int(cur.fetchone()[0])
                return {"enabled": bool(enabled), "free_codes": free_codes}
            elif scope == "cards":
...
def admin_auto_exec_set(body: AutoScopeSetIn, x_admin_password: Optional[str] = Header(None, alias="x-admin-password"), password: Optional[str] = None):
    ...
    try:
        with conn, conn.cursor() as cur:
            _ensure_settings_table(cur)
            if scope == "itunes":
                _set_flag(cur, "auto_exec_itunes", bool(body.enabled))
                try: asyncio.create_task(_itunes_autoexec_daemon())
                except Exception: pass
            elif scope == "cards":
                _set_flag(cur, "auto_exec_cards", bool(body.enabled))
                try: asyncio.create_task(_cards_autoexec_daemon())
                except Exception: pass
            else:
                _set_flag(cur, "auto_exec_api", bool(body.enabled))
...
async def _startup_scoped_autoexec():
    try:
        if not _ITUNES_DAEMON_STARTED:
            asyncio.create_task(_itunes_autoexec_daemon())
        if not _CARDS_DAEMON_STARTED:
