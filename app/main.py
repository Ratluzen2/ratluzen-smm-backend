...
# ---- Auto-exec core processing ----
def _auto_exec_one_locked(cur):
    cur.execute("""
        SELECT o.id, o.user_id, o.service_id, o.link, o.quantity, o.price, o.title, o.created_at
    ...
    if not r:
        return None
    oid, user_id, service_id, link, qty, price, title, created_at = r
    return {
        "order_id": int(oid),
        "user_id": int(user_id),
...
def _auto_exec_process_one(conn, rec):
    ...
    # Call provider (same pattern used elsewhere)
    try:
        resp = requests.post(
            PROVIDER_API_URL,
...
def _auto_exec_run(conn, limit: int = 3):
    processed = []
    runs = max(1, min(int(limit or 1), 20))
    for _ in range(runs):
        with conn, conn.cursor() as cur:
    ...
    try:
        ...
        with get_conn() as c2:
            with c2.cursor() as cur2:
                _ensure_settings_table(cur2)
                auto_enabled = _get_flag(cur2, "auto_exec_api", False)
        return {"list": out, "auto_exec_enabled": bool(auto_enabled)}
    finally:
        put_conn(conn)


# ---- Auto-exec admin routes ----
