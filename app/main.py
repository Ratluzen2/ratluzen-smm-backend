...
def admin_list_service_ids(
    ...
    try:
        with conn, conn.cursor() as cur:
            ...
            rows = cur.fetchall()
    # === insert/push notifications for user & owner ===
    try:
        with conn, conn.cursor() as _curN:
            # fetch order => user_id, title
            _curN.execute("SELECT user_id, title FROM public.orders WHERE id=%s", (oid,))
            _rowN = _curN.fetchone()
            if _rowN:
                _user_idN, _titleN = int(_rowN[0]), _rowN[1] or 'طلبك'
                # user message depends on endpoint (deliver vs reject)
                try:
                    _endpoint = request.url.path
                except Exception:
                    _endpoint = ""
                if _endpoint.endswith("/deliver"):
                    _tN = "تم تنفيذ طلبك"
                    _bN = f"تم تنفيذ {_titleN} بنجاح."
                elif _endpoint.endswith("/reject"):
                    _tN = "تم رفض طلبك"
                    _bN = f"تم رفض {_titleN} وتم رد المبلغ إن وُجد."
                else:
                    _tN = "تحديث على طلبك"
                    _bN = f"تم تحديث {_titleN}."
                # push + store for user
                _push_user(conn, _user_idN, oid, _tN, _bN)
                # also add a copy for owner (without spamming FCM if غير ضروري)
                try:
                    with conn, conn.cursor() as __cO:
                        _owner_id = _ensure_owner_user_id(__cO)
                    _push_user(conn, _owner_id, oid, _tN, f"[مالك] {_bN}")
                except Exception:
                    pass
    except Exception as e:
        logger.exception("notify inject failed: %s", e)
    # === insert/push notifications for user & owner ===
    try:
        with conn, conn.cursor() as _curN:
            # fetch order => user_id, title
