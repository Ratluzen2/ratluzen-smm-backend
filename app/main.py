...
# FastAPI & CORS
# =========================
app = FastAPI(title="SMM Backend", version="1.1.0")
app.add_middleware(
    CORSMiddleware,
...
async def _coerce_json(request: Request) -> Dict[str, Any]:
    """
    يحاول قراءة JSON أو فورم أو نص خام ثم تحويله لقاموس.
    """
    ...
    try:
        ...
        # نص خام: جرّب JSON ثم جرّب key=value&..
...
def _create_provider_order_core(cur, uid: str, service_id: int, service_name: str,
    ...
    cur.execute("""
        ...
        RETURNING id
    """, (user_id, service_name, service_id, link, quantity, Decimal(price),
...
# مسار توافقي يقبل JSON أو فورم أو نص خام وبأسماء حقول متعددة
@app.post("/api/orders/create")
async def create_provider_order_compat(request: Request):
    data = await _coerce_json(request)
    try:
        uid = (data.get("uid") or data.get("user_id") or "").strip()
        service_id = int(data.get("service_id") or data.get("service"))
        link = data.get("link") or data.get("url") or ""
        quantity = int(data.get("quantity") or data.get("qty") or data.get("amount") or 0)
        price = float(data.get("price") or data.get("cost") or 0)
        service_name = data.get("service_name") or data.get("name") or f"Service {service_id}"
        if not uid or not service_id or not link or quantity <= 0:
            raise ValueError
    except Exception:
    ...
    try:
        with conn, conn.cursor() as cur:
            oid = _create_provider_order_core(cur, uid, service_id, service_name, link, quantity, price)
        return {"ok": True, "order_id": oid}
    finally:
        put_conn(conn)
...
# مسارات توافقية لاسياسيل (تقبل JSON/فورم/نص)
@app.post("/api/asiacell/submit")
@app.post("/api/topup/asiacell")
@app.post("/api/wallet/asiacell")
async def submit_asiacell_compat(request: Request):
    data = await _coerce_json(request)
    uid = (data.get("uid") or data.get("user_id") or "").strip()
    raw = data.get("card") or data.get("code") or data.get("voucher") or data.get("value") or ""
...
def admin_approve_order(oid: int, x_admin_password: str = Header(..., alias="x-admin-password")):
    ...
    try:
        with conn, conn.cursor() as cur:
            ...
            # الطلب اليدوي (service_id None): تغيير الحالة فقط
