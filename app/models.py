...
class ServiceOrder(Base):
    ...
    service_code: Mapped[int] = mapped_column(Integer)                 # رقم/كود لدى المزوّد
    link: Mapped[str] = mapped_column(Text)                            # رابط الحساب/الفيديو
...
class PubgOrder(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    uid: Mapped[str] = mapped_column(String(64), index=True)
    pkg: Mapped[int] = mapped_column(Integer, default=60)  # 60,120,325,660,1800,3850,8100
...
class LudoOrder(Base):
    ...
    ludo_id: Mapped[str | None] = mapped_column(String(64), nullable=True)
    status: Mapped[str] = mapped_column(String(16), default="pending")
    created_at: Mapped = mapped_column(DateTime(timezone=True), server_default=func.now())
