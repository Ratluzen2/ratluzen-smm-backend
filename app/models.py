...
# استخدم نفس Base القادم من database لضمان أن create_all يعمل على نفس الـ metadata
...
class User(Base):
    ...
    # جعل uid هو المفتاح الأساسي ليتوافق مع الجدول الموجود في قاعدة البيانات
    uid: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
